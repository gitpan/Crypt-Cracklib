# This Makefile is for the Crypt::Cracklib extension to perl.
#
...
#     ABSTRACT => q[Perl interface to Alec Muffett's Cracklib.]
#     AUTHOR => q[Dan Sully <daniel@cpan.org>]
#     BUILD_REQUIRES => {  }
#     DISTNAME => q[Crypt-Cracklib]
#     LIBS => [q[-lcrack]]
#     LICENSE => q[perl]
#     MIN_PERL_VERSION => q[5.005]
#     NAME => q[Crypt::Cracklib]
#     NO_META => q[1]
#     OPTIMIZE => q[-g -Wall -Werror]
#     PREREQ_PM => { Test::More=>q[0.47], Pod::Coverage=>q[0.19], Test::Pod::Coverage=>q[1.08], ExtUtils::MakeMaker=>q[6.42] }
#     VERSION => q[1.6]
#     VERSION_FROM => q[Cracklib.pm]
#     dist => { PREOP=>q[$(PERL) -I. "-MModule::Install::Admin" -e "dist_preop(q($(DISTVNAME)))"] }
#     realclean => { FILES=>q[MYMETA.yml] }
#     test => { TESTS=>q[t/01use.t t/pod-coverage.t t/pod.t] }
...
XS_FILES = Cracklib.xs
C_FILES  = Cracklib.c
O_FILES  = Cracklib.o
H_FILES  = 
MAN1PODS = 
MAN3PODS = Cracklib.pm
...
TO_INST_PM = Cracklib.pm

PM_TO_BLIB = Cracklib.pm \
	$(INST_LIB)/Crypt/Cracklib.pm
...
DISTNAME = Crypt-Cracklib
DISTVNAME = Crypt-Cracklib-1.6
...
# Crypt::Cracklib might depend on some other libraries:
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  Cracklib.pm $(INST_MAN3DIR)/Crypt::Cracklib.$(MAN3EXT) 
...
TEST_FILE = test.pl
TEST_FILES = t/01use.t t/pod-coverage.t t/pod.t
TESTDB_SW = -d
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl interface to Alec Muffett'\''s Cracklib.</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Dan Sully &lt;daniel@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,005,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="ExtUtils::MakeMaker" VERSION="6.42" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Pod::Coverage" VERSION="0.19" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::More" VERSION="0.47" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::Pod::Coverage" VERSION="1.08" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  Cracklib.pm $(INST_LIB)/Crypt/Cracklib.pm 
